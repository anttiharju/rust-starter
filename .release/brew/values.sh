...
linux_arm_sha="$([[ -f "$repo-aarch64-unknown-linux-gnu.tar.gz" ]] && hashsum --sha256 "$repo-aarch64-unknown-linux-gnu.tar.gz" --no-names || echo "TBD")"
capture PKG_LINUX_ARM_SHA "$linux_arm_sha"
linux_intel_sha="$([[ -f "$repo-x86_64-unknown-linux-gnu.tar.gz" ]] && hashsum --sha256 "$repo-x86_64-unknown-linux-gnu.tar.gz" --no-names || echo "TBD")"
